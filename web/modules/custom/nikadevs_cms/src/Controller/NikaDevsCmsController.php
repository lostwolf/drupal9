...
class NikaDevsCmsController extends ControllerBase {
  ...
  public function release_cleanup() {
    ...
    $module_path = \Drupal\Core\Extension\ExtensionPathResolver::getPath('module', 'nikadevs_cms');
    foreach($res as $row) {
      $file_name =  FileSystem::realpath($row->uri);
      unlink($file_name);
      copy($module_path . '/img/upload_image.png', $file_name);
    }

    $database->delete('users')->condition('uid', array(0, 1), 'NOT IN')->execute();
